...
bool Index_Sq(DSqString S,DSqString T,int &pos,int i)
...
bool StrDelete_Sq(DSqString &S,int pos,int len)
    {  // 从顺序串S的第pos个字符字符开始删除长度为len的子串，并返回删除后的串S
      int i;
      if(pos<0||len<0||pos+len>S.length)  return false;  // pos和len不合理
      for(i=pos+len;i<=S.length-1;i++)           // 元素前移，删除子串
        S.str[i-len]=S.str[i];
...
{  // 用串V置换主串S中出现的所有与T相等的不重叠子串，并返回置换后的串S
      int  pos;
      while(Index_Sq(S,T,pos,0))                    // 判断T是否是S的子串
